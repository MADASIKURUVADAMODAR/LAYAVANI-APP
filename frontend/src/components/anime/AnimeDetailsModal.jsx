...
import { fetchAnimeSeasonEpisodes, getAnimeBackdropUrl, getAnimeGenres, getAnimePosterUrl, getAnimeStudios } from "../../services/animeService";
...
export default function AnimeDetailsModal({
  ...
  const source = animeDetails || anime;
  const genres = getAnimeGenres(source);
  const studios = getAnimeStudios(source);
  ...
  const streamServers = useMemo(() => ([
    `https://vidsrc.xyz/embed/anime/${source?.mal_id}/${selectedEpisode}`,
    `https://vidsrc.to/embed/anime/${source?.mal_id}/${selectedEpisode}`,
    `https://vidsrc.me/embed/anime/${source?.mal_id}/${selectedEpisode}`,
    `https://vidsrc.xyz/embed/anime/${source?.mal_id}`,
    `https://vidsrc.to/embed/anime/${source?.mal_id}`,
    `https://vidsrc.me/embed/anime/${source?.mal_id}`,
  ]), [source?.mal_id, selectedEpisode]);

  useEffect(() => {
    if (!isOpen || !source?.mal_id) return;
  ...
  }, [isOpen, source?.mal_id]);

  useEffect(() => {
    if (!isOpen || !source?.mal_id || !selectedSeason) return;

    const cacheKey = `${source.mal_id}-${selectedSeason}`;
    ...
    const loadEpisodes = async () => {
      ...
      try {
        const normalized = await fetchAnimeSeasonEpisodes(source.mal_id, selectedSeason);
        if (!active) return;
  ...
  }, [isOpen, source?.mal_id, selectedSeason]);
      ...
      >
        <div
          style={{
            position: "relative",
            minHeight: "200px",
            backgroundImage: getAnimeBackdropUrl(source)
              ? `linear-gradient(to bottom, rgba(0,0,0,0.2), rgba(9,12,20,0.98)), url(${getAnimeBackdropUrl(source)})`
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "18px", padding: "18px" }}>
          <div>
            {getAnimePosterUrl(source) ? (
              <img
                src={getAnimePosterUrl(source)}
                alt={source?.title || "Anime"}
                loading="lazy"
          ...
          <div>
            <h2 style={{ margin: "0 0 8px", fontSize: "1.8rem" }}>{source?.title || "Unknown Title"}</h2>
            <div style={{ color: "rgba(255,255,255,0.75)", marginBottom: "10px" }}>
              {(source?.year || source?.aired?.prop?.from?.year || "Unknown year")} • Episodes: {source?.episodes || "N/A"} • ⭐ {Number(source?.score || 0).toFixed(1)}
            </div>

            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "12px" }}>
              {genres.length > 0 ? genres.map((genre) => (
                <span key={genre} style={{ border: "1px solid rgba(255,255,255,0.2)", borderRadius: "999px", padding: "6px 10px", fontSize: "0.8rem" }}>
                  {genre}
                </span>
              )) : <span style={{ color: "rgba(255,255,255,0.7)" }}>Genres unavailable</span>}
            </div>

            {loading ? <div style={{ color: "rgba(255,255,255,0.72)", marginBottom: "10px" }}>Loading anime details...</div> : null}

            <p style={{ color: "rgba(255,255,255,0.85)", lineHeight: 1.65, margin: "0 0 14px" }}>
              {source?.synopsis || "No synopsis available."}
            </p>

            <div style={{ color: "rgba(255,255,255,0.75)", marginBottom: "12px" }}>
              Status: {source?.status || "N/A"} {studios.length ? `• Studio: ${studios.join(", ")}` : "• Studio: N/A"}
            </div>
